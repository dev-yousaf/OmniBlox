...
export class SuperadminService {
  ...
  async getDashboard(
    ...
    const [
      ...
      this.prisma.product.findMany({
        where: { companyId, status: 'ACTIVE', inventory: { some: { quantity: { lte: 0 } } } },
        select: { id: true, name: true, inventory: { select: { quantity: true } } },
        take: 5,
      }),
    ...
    const roleMap = new Map<string, number>(roleCounts.map((r) => [r.role, Number(r._count.id)]));
    const plansDistribution = [
      ...
      { name: 'Managers', count: roleMap.get('MANAGER') ?? 0, color: '#06aed4' },
      { name: 'Observers', count: roleMap.get('OBSERVER') ?? 0, color: '#3eb780' },
    ];
    ...
    const topCompanies = warehouses.length > 0
      ? warehouses.map((w) => ({
          id: w.id,
          name: w.name,
          logo: undefined,
      ...
      : [{ id: 'none', name: 'No warehouses', logo: undefined, plan: '', usersCount: 0 }];
  ...
  private async getActivityChart(companyId: string, period: string, now: Date): Promise<BarChartItem[]> {
    ...
    if (period === '1W') {
      const rows = await this.prisma.$queryRawUnsafe(
        ...
        companyId, new Date(now.getTime() - 7 * 86400000),
      ) as Array<{ day: Date; count: number }>;

      const map = new Map<number, number>(rows.map((r) => [r.day.getTime(), r.count]));
      const result: BarChartItem[] = [];
      for (let i = 6; i >= 0; i--) {
        const d = new Date(now.getFullYear(), now.getMonth(), now.getDate() - i);
        result.push({ day: dayNames[d.getDay()], count: map.get(d.getTime()) ?? 0 });
      }
      return result;
    }
    ...
    if (period === '1M') {
      const rows = await this.prisma.$queryRawUnsafe(
        ...
        companyId, new Date(now.getTime() - 28 * 86400000),
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        const wkStart = new Date(ws.getFullYear(), ws.getMonth(), ws.getDate() - ws.getDay());
        result.push({ day: `W${4 - i}`, count: map.get(wkStart.getTime()) ?? 0 });
      }
      return result;
    }
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      result.push({ day: monthNames[m.getMonth()], count: map.get(m.getTime()) ?? 0 });
    }
    return result;
  }

  private async getExpensesByCategory(companyId: string, periodAgo: Date): Promise<MonthlyRevenueItem[]> {
    const rows = await this.prisma.$queryRawUnsafe(
