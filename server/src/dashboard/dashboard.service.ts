...
export class DashboardService {
  ...
  async getData(
    ...
    const [
      ...
      this.prisma.sale.aggregate({
        where: {
          companyId,
          saleDate: { gte: prevRange.start, lte: prevRange.end },
          status: { not: 'CANCELLED' },
        },
        _sum: { totalAmount: true },
      }),
      ...
      this.prisma.expense.aggregate({
        where: {
          companyId,
          expenseDate: { gte: prevRange.start, lte: prevRange.end },
          status: { not: 'REJECTED' },
        },
        _sum: { amount: true },
      }),
    ...
    const purchaseReturn = Number(
      currentPurchaseReturnAgg._sum.totalAmount || 0,
    );
    const prevPurchaseReturn = Number(
      prevPurchaseReturnAgg._sum.totalAmount || 0,
    );
    ...
    return {
      ...
      invoiceDueChange: this.pctChange(
        Number(invoiceDueAgg._sum.totalAmount || 0),
        prevInvoiceDue,
      ),
  ...
  private async getLowStockProducts(companyId: string) {
    ...
    const stockMap = new Map<
      string,
      { product: ProductInfo; totalQty: number }
    >();
    for (const inv of inventory) {
      const existing = stockMap.get(inv.productId) || {
        product: inv.product,
        totalQty: 0,
      };
  ...
  private async getCustomerStats(
    ...
    const customerIds = [
      ...new Set(salesInPeriod.map((s) => s.customerId)),
  ...
  async getChartForPeriod(companyId: string, period: string = '1Y') {
    ...
    const [salesAgg, prevSalesAgg, expensesAgg, prevExpensesAgg] =
      await Promise.all([
        this.prisma.sale.aggregate({
          where: {
            companyId,
        ...
        this.prisma.sale.aggregate({
          where: {
            companyId,
            saleDate: {
              gte: this.getPreviousPeriodRange(period, now).start,
              lte: this.getPreviousPeriodRange(period, now).end,
            },
            status: { not: 'CANCELLED' },
          },
          _sum: { totalAmount: true },
