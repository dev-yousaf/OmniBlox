  ...
  function ProductForm({ initialData, isEdit = false, productId, onSuccess, hideWarehouse = false, formId, onSubmittingChange = () => {}, showSubmit = true }, ref) {
    ...
    useEffect(() => {
      const loadData = async () => {
        try {
          const [categoriesData, brandsData, unitsData, subCatsData, warrantiesData, whData] = await Promise.all([
            getCategories(),
            getBrands(),
            getUnits(),
            getSubCategories(),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        const productData: Record<string, unknown> = {
          ...
          attributes: formData.hasVariants
            ? (parentAttributes ?? formData.attributes)
            : undefined,
          variants:
            !isEdit && formData.hasVariants && variantsPayload.length > 0
              ? variantsPayload
              : undefined,
        };
